...
def get_user_details(request: schemas.UserDetailRequest):
    ...
    return schemas.UserDetailResponse(
        ...
        email=scim_user.get("emails", [{}])[0].get("value") if scim_user.get("emails") else None,
...
def assign_role_to_user(request: schemas.RoleAssignmentRequest):
    ...
    return {"success": True, "message": f"Role '{request.role_name}' assigned to user '{request.username}'"}
...
def remove_role_from_user(request: schemas.RoleRemovalRequest):
    ...
    return {"success": True, "message": f"Role '{request.role_name}' removed from user '{request.username}'"}
...
def bulk_assign_roles(request: schemas.BulkRoleAssignmentRequest):
    ...
    for assignment in request.assignments:
        try:
            result = oracle.assign_role_to_user(assignment.username, assignment.role_name)
            if result.get("success"):
                successful += 1
                results.append(schemas.OperationStatus(
                    success=True,
...
@router.post("/users/data-security/bulk-assign", response_model=schemas.BulkOperationResponse)
...
def get_areas_of_responsibility(
    ...
    if not result.get("success"):
        raise HTTPException(status_code=502, detail=f"Failed to get areas of responsibility: {result.get('error')}")
...
def assign_aor_to_user(request: schemas.AORAssignmentRequest):
    ...
    return {"success": True, "message": f"AOR '{request.aor_name}' assigned to user '{request.username}'"}
...
@router.post("/areas-of-responsibility/bulk-assign", response_model=schemas.BulkOperationResponse)
...
async def upload_excel_file(
    ...
    operation_type: str = Query(..., description="Type of operation: role_assignment, data_security, aor_assignment"),
    instance_url: str = Query(..., description="Oracle Instance URL"),
    oracle_username: str = Query(..., description="Oracle API username"),
    oracle_password: str = Query(..., description="Oracle API password")
):
        ...
        for index, row in df.iterrows():
            try:
                if operation_type == "role_assignment":
                    ...
                    result = {"success": False, "error": f"Unsupported operation type: {operation_type}"}
...
def update_user_password(request: schemas.PasswordUpdateRequest):
    ...
    return {"success": True, "message": f"Password updated for user '{request.username}'"}
...
def search_users(request: schemas.UserSearchRequest):
    ...
    result = oracle.get_all_users()
    if not result.get("success"):
        raise HTTPException(status_code=502, detail=f"Search failed: {result.get('error')}")
